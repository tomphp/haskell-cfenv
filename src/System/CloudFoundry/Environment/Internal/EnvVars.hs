...
stringFromEnv = liftIO . eitherLookupEnv >=> liftEither

-- TODO: Test me!!!
stringFromEnvWithDefault :: String -> String -> EitherT String IO String
stringFromEnvWithDefault def =
    liftIO . eitherLookupEnv >=> liftEither . eitherDefault def
  where
    eitherDefault def = Right . fromRight def

-- TODO: Test me!!!
numberFromEnv :: String -> EitherT String IO Int
numberFromEnv envName =
    stringFromEnv envName >>= liftEither . readEither
  where
...
eitherLookupEnv envName = do
    value <- lookupEnv envName
    return $ maybeToEither notFoundMessage value
  where
    notFoundMessage = envName ++ " is not set."
